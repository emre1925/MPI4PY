...
tsleep=0.018
p=0.4
#debugVector = np.zeros([numberOfWorkers,numberOfIter +1 ])
#debugTag =[]
#append=1
if rank == pm:
        ts = time.time()
        ...
        for i in range (1,numberOfIter+2):
            ...
            if i == numberOfIter + 1:
                ...
                while (lastTagCounter < numberOfWorkers):
                    ...
                    #debugVector[status.Get_source() - 1][status.Get_tag() - 1] = 1
                    totalrecv += 1
                    if status.Get_tag() == i:
                        totalSent += int(Y[0][0])
                        lastTagCounter += 1
        ...
        while iterIndex <= numberOfIter+1:
            if iterIndex == numberOfIter +1:
                ...
                if MPI.Request.Test(req)  and iterIndex <=numberOfIter and sleepFlag:
                    ...
                    if iterIndex <= numberOfIter:
                        req = comm.Irecv(message, source=pm)
                if iterIndex == compTag and counter < numberOfComp and iterIndex <= numberOfIter and sleepFlag:
                    #debugVector2[iterIndex] = 1
                    comm.Send(Y, dest=pm, tag=iterIndex)
                    counter += 1
                    sendCounter +=1